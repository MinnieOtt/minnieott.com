  ...
  app.post("/api/mochi", async (req, res) => {
    try {
      ...
      if (!process.env.GEMINI_API_KEY) {
        ...
        } else if (lastUserMessage.includes('book') || lastUserMessage.includes('meeting') || lastUserMessage.includes('schedule') || lastUserMessage.includes('calendar') || lastUserMessage.includes('appointment')) {
          simulatedText = "I would love to help you book a meeting with Minnie! 🥞 You can schedule a 1:1 consultation, project sync, or advisory session directly on her [Google Appointment Calendar](https://calendar.app.google/MCnhZcK56rLJ7fnk8). Just pick a time that works best for you!";
